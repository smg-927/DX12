...
void BasicTessellationApp::BuildQuadPatchGeometry()
{
	...
	auto indices16 = sphere.GetIndices16();
	...
	CopyMemory(geo->VertexBufferCPU->GetBufferPointer(), sphere.Vertices.data(), vbByteSize);

	ThrowIfFailed(D3DCreateBlob(ibByteSize, &geo->IndexBufferCPU));
	CopyMemory(geo->IndexBufferCPU->GetBufferPointer(), indices16.data(), ibByteSize);

	geo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), sphere.Vertices.data(), vbByteSize, geo->VertexBufferUploader);
	...
	quadSubmesh.IndexCount = (UINT)indices16.size();
