...
private:
	...
	std::vector<D3D12_INPUT_ELEMENT_DESC> mCylinderInputLayout;
...
void TreeBillboardsApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mPSOs["opaquecylinder"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::OpaqueCylinder]);
...
void TreeBillboardsApp::LoadTextures()
{
	...
	cylinderTex->Filename = L"../../../Textures/bricks.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void TreeBillboardsApp::BuildShadersAndInputLayouts()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");

	mShaders["CylinderVS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Cylinder.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["CylinderGS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Cylinder.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["CylinderPS"] = d3dUtil::CompileShader(L"..\\Chap12_1\\Shaders\\Cylinder.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
void TreeBillboardsApp::BuildCylinderGeometry()
{
	...
	{
		XMFLOAT3(5.0000000f, 0.0f,  0.0000000f),
		XMFLOAT3(3.5355340f, 0.0f,  3.5355340f),
		XMFLOAT3(0.0000000f, 0.0f,  5.0000000f),
		XMFLOAT3(-3.5355340f, 0.0f,  3.5355340f),
		XMFLOAT3(-5.0000000f, 0.0f,  0.0000000f),
		XMFLOAT3(-3.5355340f, 0.0f, -3.5355340f),
		XMFLOAT3(0.0000000f, 0.0f, -5.0000000f),
	...
	std::array<CylinderVertex, 8> vertices;
	for (UINT i = 0; i < 8; ++i)
	{
		vertices[i].Pos = p[i];
		vertices[i].Height = 10;
	}
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(CylinderVertex);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);
	...
	geo->VertexByteStride = sizeof(CylinderVertex);
	geo->VertexBufferByteSize = vbByteSize;
	geo->IndexFormat = DXGI_FORMAT_R16_UINT;
	geo->IndexBufferByteSize = ibByteSize;
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	cylinderPsoDesc.InputLayout = { mCylinderInputLayout.data(), (UINT)mCylinderInputLayout.size() };
	cylinderPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;

	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&cylinderPsoDesc, IID_PPV_ARGS(&mPSOs["opaquecylinder"])));
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::OpaqueCylinder].push_back(cylinderRitem.get());
