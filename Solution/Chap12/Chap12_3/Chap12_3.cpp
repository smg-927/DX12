...
private:
	...
	void CalcExplodeTime(const GameTimer& gt);
...
private:
	...
	std::vector<D3D12_INPUT_ELEMENT_DESC> mSphereInputLayout;
	...
	float explodeSpeed = 0.0f;
	float gExplodeTime = 0.0f;
	float gExplodeStartTime = 0.0f;
	POINT mLastMousePos;
};
...
void TreeBillboardsApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mPSOs["opaquesphere"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::OpaqueSphere]);
...
void TreeBillboardsApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	{
		explodeSpeed = 200.0f;
		gExplodeStartTime = gt.TotalTime();

		if(gExplodeTime < 1.0f)
			gExplodeTime = gExplodeStartTime;
	}
...
void TreeBillboardsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.explodeSpeed = explodeSpeed;
	mMainPassCB.gExplodeStartTime = gExplodeStartTime;
	mMainPassCB.gExplodeTime = gExplodeTime;
...
void TreeBillboardsApp::CalcExplodeTime(const GameTimer& gt)
{
	if (gExplodeStartTime != 0.0f)
	{
		gExplodeTime += gt.DeltaTime();

		if(explodeSpeed > 4.0f)
			explodeSpeed -= gt.DeltaTime()*10;
	}
}
...
void TreeBillboardsApp::LoadTextures()
{
	...
	sphereTex->Name = "sphereTex";
	sphereTex->Filename = L"../../../Textures/bricks.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void TreeBillboardsApp::BuildDescriptorHeaps()
{
	...
	auto sphereTex = mTextures["sphereTex"]->Resource;
	...
	srvDesc.Format = sphereTex->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = -1;
	md3dDevice->CreateShaderResourceView(sphereTex.Get(), &srvDesc, hDescriptor);
}
...
void TreeBillboardsApp::BuildShadersAndInputLayouts()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");

	mShaders["sphereVS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Sphere.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["sphereGS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Sphere.hlsl", nullptr, "GS", "gs_5_0");
	mShaders["spherePS"] = d3dUtil::CompileShader(L"..\\Chap12_3\\Shaders\\Sphere.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
void TreeBillboardsApp::BuildSphereGeometry()
{
	...
	{
		GeometryGenerator::Vertex v = sphere.Vertices[i];
		vertices[i].Pos = v.Position;
		vertices[i].Normal = v.Normal;
		vertices[i].TexC = v.TexC;
	}
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	spherePsoDesc.InputLayout = { mSphereInputLayout.data(), (UINT)mSphereInputLayout.size() };
	spherePsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;

	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&spherePsoDesc, IID_PPV_ARGS(&mPSOs["opaquesphere"])));
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::OpaqueSphere].push_back(sphereRitem.get());
