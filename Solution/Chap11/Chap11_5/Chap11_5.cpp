...
#include "..\Chap11_Stencil\FrameResource.h"
...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetPipelineState(mPSOs["wall"].Get());
...
void StencilApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"..\\Chap11_4\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"..\\Chap11_4\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"..\\Chap11_4\\Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
{
	std::ifstream fin("../Chap11_Stencil/Models/skull.txt");

	if (!fin)
	{
		MessageBox(0, L"../Chap11_Stencil/Models/skull.txt not found.", 0, 0);
		return;
	}
...
void StencilApp::BuildPSOs()
{
	...
	mirrorDSS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO; // D3D12_DEPTH_WRITE_MASK_ZERO 원래 버전
	...
	auto wallDS = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	wallDS.DepthEnable = true;
	wallDS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
	wallDS.DepthFunc = D3D12_COMPARISON_FUNC_LESS;

	D3D12_GRAPHICS_PIPELINE_STATE_DESC wallPsoDesc = opaquePsoDesc;
	opaquePsoDesc.DepthStencilState = wallDS;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&wallPsoDesc, IID_PPV_ARGS(&mPSOs["wall"])));

	auto skullDS = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	skullDS.DepthEnable = true;
	skullDS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
	skullDS.DepthFunc = D3D12_COMPARISON_FUNC_LESS;

	D3D12_GRAPHICS_PIPELINE_STATE_DESC skullPsoDesc = opaquePsoDesc;
	opaquePsoDesc.DepthStencilState = skullDS;
