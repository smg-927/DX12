...
private:
	...
	std::unordered_map<std::string, UINT> mTexSrvIndex;
...
static inline std::string BoltName(int frame) {
	char buf[16];
	std::snprintf(buf, sizeof(buf), "Bolt%03d", frame); // Bolt001 ~ Bolt060
...
void BlendApp::AnimateMaterials(const GameTimer& gt)
{
	...
	const int fps = 60;
	const int frames = 60;
	int frame = (int)(gt.TotalTime() * fps) % frames + 1; // 1..60

	const std::string name = BoltName(frame);
	auto jt = mTexSrvIndex.find(name);
	if (jt != mTexSrvIndex.end()) {
		boltMat->DiffuseSrvHeapIndex = jt->second;
	}
}
...
void BlendApp::LoadTextures()
{
	...
	for (int i = 1; i <= 60; ++i) {
		...
		// 1) 텍스처 이름: Bolt001 ~ Bolt060
		if (i < 10)        boltTex->Name = "Bolt00" + std::to_string(i);
		else if (i < 100)  boltTex->Name = "Bolt0" + std::to_string(i);
		else               boltTex->Name = "Bolt" + std::to_string(i); // (확장 대비)

		// 2) 파일 경로(wide) 만들기
		//    만약 파일명이 .dds라면 확장자 붙이세요 (필요 시)
		const std::wstring base = L"../../../Exercise_Media/BoltAnim/";
		std::wstring wname(boltTex->Name.begin(), boltTex->Name.end());
		boltTex->Filename = base + wname + L".dds";

		// 3) DDS 로드 (owner 일치!)
		ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
			md3dDevice.Get(),
			mCommandList.Get(),
			boltTex->Filename.c_str(),
			boltTex->Resource,         // 리소스는 boltTex의 멤버
			boltTex->UploadHeap        // 업로드 힙도 boltTex의 멤버
		));
...
{
	const UINT texCount = static_cast<UINT>(mTextures.size());
	//
	// Create the SRV heap.
	//
	...
	auto makeSRV = [&](const std::string& name) {
		auto& up = mTextures.at(name);
		auto tex = up->Resource.Get();
		if (!tex) { /* 로그/예외 처리 */ return; }

		const auto tdesc = tex->GetDesc();
		...
		srv.Texture2D.MipLevels = tdesc.MipLevels;   // ✅ -1 금지

		md3dDevice->CreateShaderResourceView(tex, &srv, hCpu);

		mTexSrvIndex[name] = index++;                // 이름→슬롯 인덱스 매핑
		hCpu.Offset(1, mCbvSrvDescriptorSize);       // 다음 슬롯으로
	};
    ...
    for (int i = 1; i <= 60; ++i) {
        const std::string name =
            (i < 10) ? "Bolt00" + std::to_string(i) :
            (i < 100) ? "Bolt0" + std::to_string(i) :
                        "Bolt"   + std::to_string(i);
        makeSRV(name);
    }
}
...
void BlendApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"..\\..\\Chap11\\Chap11_7\\Shaders\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"..\\..\\Chap11\\Chap11_7\\Shaders\\Default.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"..\\..\\Chap11\\Chap11_7\\Shaders\\Default.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData cylinder = geoGen.CreateCylinder(3.0f, 3.0f, 6.0f, 30, 5);
	...
	for (size_t i = 0; i < cylinder.Vertices.size(); ++i)
	{
		...
		if (vertices[i].Normal.y > 0.1f) vertices[i].TexC = { 0.f, 0.f };
	}
...
void BlendApp::BuildPSOs()
{
	...
	//
	// PSO for additive (lightning, glow, etc.)
	//
	D3D12_GRAPHICS_PIPELINE_STATE_DESC additivePsoDesc = opaquePsoDesc;

	// 픽셀 셰이더: 알파테스트 버전 말고, 일반(opaque) PS 사용
	additivePsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["opaquePS"]->GetBufferPointer()),
		mShaders["opaquePS"]->GetBufferSize()
	...
	addBlend.LogicOpEnable = FALSE;
	addBlend.SrcBlend = D3D12_BLEND_ONE;       // Cdst = Csrc*1 + Cdst*1
	addBlend.DestBlend = D3D12_BLEND_ONE;
	addBlend.BlendOp = D3D12_BLEND_OP_ADD;
	addBlend.SrcBlendAlpha = D3D12_BLEND_ONE;       // 알파는 의미 적으니 ONE/ONE
	...
	additivePsoDesc.BlendState.RenderTarget[0] = addBlend;

	// 반투명 이펙트에서는 보통 깊이 쓰기 OFF
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
		&additivePsoDesc, IID_PPV_ARGS(&mPSOs["additive"])));
}
...
void BlendApp::BuildMaterials()
{
	...
	bolt->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f);
	bolt->FresnelR0 = XMFLOAT3(0.01f, 0.01f, 0.01f);
	bolt->Roughness = 0.125f;

	grass->DiffuseSrvHeapIndex = mTexSrvIndex["grassTex"];
	water->DiffuseSrvHeapIndex = mTexSrvIndex["waterTex"];
	bolt->DiffuseSrvHeapIndex = mTexSrvIndex["Bolt003"];
