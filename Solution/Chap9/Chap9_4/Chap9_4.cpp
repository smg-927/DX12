...
#include "../Chap9_Crate/FrameResource.h"
...
void CrateApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->OMSetRenderTargets(1, &cbbview, true, &dsview);
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors = 2; // <-- 두 개 이상의 SRV를 쓸 예정이면 여기를 충분히 크게
	srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	srvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	ThrowIfFailed(md3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&mSrvDescriptorHeap)));

	//
	// Ensure descriptor size is cached (do this earlier in init if you prefer).
	//
	...
	// --- woodCrateTex SRV ---
	auto woodIt = mTextures.find("woodCrateTex");
	if (woodIt == mTextures.end() || woodIt->second == nullptr || woodIt->second->Resource.Get() == nullptr)
		throw std::runtime_error("woodCrateTex not found or null");

	ComPtr<ID3D12Resource> woodCrateTex = woodIt->second->Resource;
	...
	// --- bricksTex SRV ---
	auto bricksIt = mTextures.find("bricksTex");
	if (bricksIt == mTextures.end() || bricksIt->second == nullptr || bricksIt->second->Resource.Get() == nullptr)
		throw std::runtime_error("bricksTex not found or null");

	ComPtr<ID3D12Resource> bricksTex = bricksIt->second->Resource;

	// 재사용 전에 srvDesc를 다시 초기화해도 안전
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 8> CrateApp::GetStaticSamplers()
{
	...
	const CD3DX12_STATIC_SAMPLER_DESC Border(
		...
		0.0f,                                   // mipLODBias
		0,                                      // maxAnisotropy (0 for non-aniso)
		D3D12_COMPARISON_FUNC_ALWAYS,           // comparisonFunc
		D3D12_STATIC_BORDER_COLOR_OPAQUE_WHITE, // borderColor (enum)
