...
#include "..\Chap7_ShapesDemo\FrameResource.h"
...
{
    // We no longer update per-object constant buffers because per-object data
    // (World matrix) is uploaded via root constants at draw time.
    // Optionally clear the NumFramesDirty flags so we don't do redundant work.
...
void ShapesApp::BuildDescriptorHeaps()
{
    ...
    // We no longer need per-object CBV descriptors in the heap.
    // Only need one CBV per frame resource for the per-pass CB (gNumFrameResources).
...
{
    // We only create per-pass CBVs (one for each frame resource).
    UINT passCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(PassConstants));

    for (int frameIndex = 0; frameIndex < gNumFrameResources; ++frameIndex)
    {
        // Use the PassCB resource (not ObjectCB)
        auto passCB = mFrameResources[frameIndex]->PassCB->Resource();
        D3D12_GPU_VIRTUAL_ADDRESS cbAddress = passCB->GetGPUVirtualAddress();

        // Offset to the pass cbv in the descriptor heap (starts at mPassCbvOffset which we set to 0)
...
void ShapesApp::BuildRootSignature()
{
    ...
    // Now we use:
    //  - slot 0 : 16 x 32-bit root constants -> per-object world matrix (register b0)
    //  - slot 1 : descriptor table for pass CBV (register b1)
	CD3DX12_ROOT_PARAMETER slotRootParameter[2];

    // slot 0 : per-object world matrix as 16 32-bit constants (maps to b0)
    slotRootParameter[0].InitAsConstants(16, 0);
    // slot 1 : per-pass CBV descriptor table (unchanged)
...
{
	mShaders["standardVS"] = d3dUtil::CompileShader(L"..\\Chap7_2\\Shaders\\color.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"..\\Chap7_2\\Shaders\\color.hlsl", nullptr, "PS", "ps_5_1");
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    for(size_t i = 0; i < ritems.size(); ++i)
    {
        ...
        cmdList->SetGraphicsRoot32BitConstants(0, 16, &worldT, 0);
